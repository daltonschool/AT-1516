        ...
        try {
            if (sleepUntil > System.nanoTime()) ;
                ...
                if (Math.abs(BR.getCurrentPosition() - encoderZero) < 500) { //not used right now
                    ...
                    state = 8; //switch back to 5 when we figure out what's wrong with camera.
                    sleep(1000);
                }
            } else if (state == 4) {
                lift.setPosition(0);
...
//                Alliance[] beacon = findBeaconColors(rgbLevels);
                ...
                sleep(2000);
                state = 6;
            } else if (state == 8) { //shitty insert I'm so sorry
                lift.setPosition(1);
                sleep(2000);
                state = 9;
            }
            if (state == 6) {
                ...
                if (Math.abs(BR.getCurrentPosition() - encoderZero) < 1200) {
                    BL.setPower(-0.25);
                    BR.setPower(-0.25);
                    FL.setPower(-0.25);
                    FR.setPower(-0.25);
